...
$result = mysql_query("SELECT DATE_FORMAT(`Date` ,'%d/%m/%y') AS meio, ROUND(AVG(`Temp Out`),0) FROM dados WHERE `Date` between '$data1' and '$data2' GROUP BY `Date`");
$dados   = array();
...
$result2 = mysql_query("SELECT DATE_FORMAT(`Date` ,'%d/%m/%y') AS meio, ROUND(AVG(`Out Hum`),0) FROM dados WHERE `Date` between '$data1' and '$data2' GROUP BY `Date`");
$dados2   = array();
...
$result3 = mysql_query("SELECT DATE_FORMAT(`Date` ,'%d/%m/%y') AS meio, ROUND(SUM(`Rain`),0) FROM dados WHERE `Date` between '$data1' and '$data2' GROUP BY `Date`");
$dados3   = array();
while ($row3 = mysql_fetch_assoc($result3)) {
            $dados3[]   = $row3["ROUND(SUM(`Rain`),0)"];
	    $label3[] = $row3["meio"];
        }
...
$result4 = mysql_query("SELECT DATE_FORMAT(`Date` ,'%d/%m/%y') AS meio, ROUND(AVG(`Wind Speed`),0) FROM dados WHERE `Date` between '$data1' and '$data2' GROUP BY `Date`");
$dados4   = array();
while ($row4 = mysql_fetch_assoc($result4)) {
            $dados4[]   = $row4["ROUND(AVG(`Wind Speed`),0)"];
	    $label4[] = $row4["meio"];
        }
...
$result5 = mysql_query("SELECT DATE_FORMAT(`Date` ,'%d/%m/%y') AS meio, ROUND(AVG(`Solar Rad`),0) FROM dados WHERE `Solar Rad` <> 0 and `Date` between '$data1' and '$data2' GROUP BY `Date`");
$dados5   = array();
while ($row5 = mysql_fetch_assoc($result5)) {
            $dados5[]   = $row5["ROUND(AVG(`Solar Rad`),0)"];
	    $label5[] = $row5["meio"];
...
//capturando índice uv 
$result6 = mysql_query("SELECT DATE_FORMAT(`Date` ,'%d/%m/%y') AS meio, ROUND(MAX(`UV Index`),0) FROM dados WHERE `Date` between '$data1' and '$data2' GROUP BY `Date`");
$dados6   = array();
while ($row6 = mysql_fetch_assoc($result6)) {
            $dados6[]   = $row6["ROUND(MAX(`UV Index`),0)"];
	    $label6[] = $row6["meio"];
...
$result7 = mysql_query("SELECT DATE_FORMAT(`Date` ,'%d/%m/%y') AS meio, ROUND(MAX(`Hi Temp`),0) FROM dados WHERE `Date` between '$data1' and '$data2' GROUP BY `Date`");
$dados7   = array();
while ($row7 = mysql_fetch_assoc($result7)) {
            $dados7[]   = $row7["ROUND(MAX(`Hi Temp`),0)"];
	    $label7[] = $row7["meio"];
        }
...
$result8 = mysql_query("SELECT DATE_FORMAT(`Date` ,'%d/%m/%y') AS meio, ROUND(MIN(`Low Temp`),0) FROM dados WHERE `Date` between '$data1' and '$data2' GROUP BY `Date`");
$dados8   = array();
while ($row8 = mysql_fetch_assoc($result8)) {
            $dados8[]   = $row8["ROUND(MIN(`Low Temp`),0)"];
	    $label8[] = $row8["meio"];
        }

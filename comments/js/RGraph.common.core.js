...
{var gutterTop=obj.gutterTop;var gutterBottom=obj.gutterBottom;var context=obj.context;var x=prop['axis.x'];var y=obj.properties['chart.gutter.top'];var min=prop['axis.min']?prop['axis.min']:0;var max=prop['axis.max'];var color=prop['axis.color']?prop['axis.color']:'black';var title=prop['axis.title']?prop['axis.title']:'';var title_color=prop['axis.title.color']?prop['axis.title.color']:color;var label_color=prop['axis.text.color']?prop['axis.color']:color;var height=obj.canvas.height-obj.gutterBottom-obj.gutterTop;var num=prop['axis.numticks']?prop['axis.numticks']:10;var gap=height/num;var font=prop['axis.font']?prop['axis.font']:'Arial';var size=prop['axis.text.size']?prop['axis.text.size']:10;var align=typeof(prop['axis.align'])=='string'?prop['axis.align']:'left';var formatter=prop['axis.scale.formatter'];var units_pre=prop['axis.units.pre'];var units_post=prop['axis.units.post'];context.strokeStyle=color;context=obj.context;context.beginPath();context.moveTo(AA(obj,x),y);context.lineTo(AA(obj,x),y+height);context.stroke();context.beginPath();for(var i=0;i<=num;++i){context.moveTo(align=='right'?x+3:x-3,AA(obj,y+(gap*i)));context.lineTo(x,AA(obj,y+(gap*i)));}
context.stroke();context.fillStyle=label_color;context.beginPath();var text_len=0;for(var i=0;i<=5;++i){var original=((max-min)*((5-i)/5))+min;var text=RGraph.number_format(obj,original,units_pre,units_post);var text=String(typeof(formatter)=='function'?formatter(obj,original):text);var text_len=Math.max(text_len,context.measureText(text).width);RGraph.Text(context,font,size,x-(align=='right'?-5:5),gutterTop+((height/5)*i),text,'center',align=='right'?'left':'right');}
context.stroke();if(title){context.beginPath();context.fillStyle=title_color
RGraph.Text(context,font,size+2,align=='right'?x+size+text_len:x-size-text_len,height/2+gutterTop,title,'center','center',null,align=='right'?90:-90);context.stroke();}}
RGraph.MeasureText=function(text,bold,font,size)
{var div=document.createElement('DIV');div.innerHTML=text;div.style.position='absolute';div.style.top=0;div.style.left=0;div.style.backgroundColor='red';div.style.fontFamily=font;div.style.fontWeight=bold?'bold':'normal';div.style.fontSize=size+'pt';document.body.appendChild(div);var size=[div.offsetWidth,div.offsetHeight];document.body.removeChild(div);return size;}
